  ...
  List<UniversalDataClass> parseDataClasses() {
    ...
    entities.forEach((key, value) {
      ...
      if (value.containsKey(_propertiesConst)) {
        ...
        if (value.containsKey(_requiredConst)) {
          final required = value[_requiredConst] as List<dynamic>;
        ...
        bool containsOf = false;
        for (final ofConst in _ofConsts) {
          if (value.containsKey(ofConst)) {
            containsOf = true;
            break;
          }
        }
      ...
      for (final ofConst in _ofConsts) {
        if (value.containsKey(ofConst)) {
          containedKey = ofConst;
          break;
        }
      }

      bool hasPropertyName = value.containsKey(_discriminatorConst) &&
          value[_discriminatorConst].containsKey(_propertyNameConst);

      if (containedKey.isNotEmpty && hasPropertyName) {
        if (value.containsKey(containedKey)) {
          for (final map in value[containedKey] as List<dynamic>) {
      ...
      final propertyName = hasPropertyName
          ? value[_discriminatorConst][_propertyNameConst].toString()
          : null;

      OfType? ofType =
          containedKey.isNotEmpty ? OfType.fromString(containedKey) : null;
      ...
      dataClasses.add(
        UniversalComponentClass(
          name: key,
          imports: imports,
          parameters: ofRefs != null ? [] : parameters,
          ofRefs: ofRefs,
          ofType: ofType,
          unionKey: propertyName,
          description: value[_descriptionConst]?.toString(),
        ),
      );
    });
    ...
    // check for `allOf`
    final allOfClasses = dataClasses.where((dc) =>
        dc is UniversalComponentClass &&
        dc.ofRefs != null &&
        dc.ofType == OfType.allOf);
    for (final allOfClass in allOfClasses) {
      if (allOfClass is! UniversalComponentClass) {
        continue;
      }
      final refs = allOfClass.ofRefs!.refs;
      ...
      allOfClass.parameters.addAll(allOfClass.ofRefs!.properties);
    }

    // add parentClass for of
    for (final dataClass in dataClasses) {
      if (dataClass is! UniversalComponentClass) {
        continue;
      }
      if (dataClass.ofRefs != null) {
        final refs = dataClass.ofRefs!.refs;
        final foundClasses = dataClasses.where((e) => refs.contains(e.name));
        for (final element in foundClasses) {
          if (element is UniversalComponentClass) {
            element.parentClass = dataClass.name;
            element.imports.add(dataClass.name);
          }
        }
      }
    }
    ...
    for (final dataClass in dataClasses) {
      ...
      for (final param in dataClass.parameters) {
        if (param.nullable) {
          param.wrappingCollections = param.wrappingCollections.map((e) {
            if (e == UniversalCollections.map) {
              return UniversalCollections.nullableMap;
            } else if (e == UniversalCollections.list) {
              return UniversalCollections.nullableList;
            } else {
              return e;
            }
          }).toList();
          ...
          param.wrappingCollections = param.wrappingCollections.map((e) {
            if (e == UniversalCollections.nullableMap) {
              return UniversalCollections.map;
            } else if (e == UniversalCollections.nullableList) {
              return UniversalCollections.list;
            } else {
              return e;
            }
          }).toList();
