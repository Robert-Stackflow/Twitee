...
class Utils {
  ...
  static int parseToInt(dynamic value) {
    if (value is int) {
      ...
      try {
        ...
        ILogger.error("Twitee", "Failed to parse int from $value", e, t);
        return 0;
      }
    } else {
      return 0;
  ...
  static compareVersion(String a, String b) {
    try {
      ...
      ILogger.error(
          "Twitee", "Failed to compare version between $a and $b", e, t);
      return a.compareTo(b);
    }
  }
  ...
  static getReleases({
    ...
    await GithubApi.getReleases("Robert-Stackflow", "Twitee")
        .then((releases) async {
      if (showLoading) {
            ...
            DialogBuilder.showConfirmDialog(
              ...
              onTapConfirm: () async {
                if (ResponsiveUtil.isAndroid()) {
                  ...
                  ILogger.info("Twitee", "Get android asset: $androidAssset");
  ...
  static localAuth({Function()? onAuthed}) async {
    ...
    try {
      ...
      ILogger.error("Twitee", "Failed to local authenticate", e, t);
    }
  }
  ...
  static Future<void> initTray() async {
    ...
    if (Platform.environment.containsKey('FLATPAK_ID') ||
      ...
      await trayManager.setIcon('assets/logo-round.ico');
    } else {
    ...
    entry = OverlayEntry(
      builder: (context) {
        return KeyboardWidget(
          bindings: defaultTwiteeShortcuts,
          callbackOnHide: () {
