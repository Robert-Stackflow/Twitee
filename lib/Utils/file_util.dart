...
import 'package:twitee/Utils/website_util.dart';
...
class FileUtil {
  ...
  static Future<FilePickerResult?> pickFiles({
    ...
    try {
      ...
      ILogger.error("Twitee", "Failed to pick files", e, t);
  ...
  static Future<String?> saveFile({
    ...
    try {
      ...
      ILogger.error("Twitee", "Failed to save file", e, t);
      IToast.showTop(S.current.pleaseGrantFilePermission);
    } finally {
      appProvider.preventLock = false;
    }
  ...
  static Future<String?> getDirectoryPath({
    ...
    try {
      ...
      ILogger.error("Twitee", "Failed to get directory path", e, t);
      IToast.showTop(S.current.pleaseGrantFilePermission);
    } finally {
      appProvider.preventLock = false;
    }
    ...
    if (ResponsiveUtil.isDesktop()) {
      ...
      if (filePath != null) {
        ...
        try {
          ...
          ILogger.error("Twitee", "Failed to zip logs", e, t);
          IToast.showTop(S.current.exportFailed);
        } finally {
          if (showLoading) {
    ...
    if (files.isNotEmpty) {
      for (var file in files) {
        if (file is File) {
          ...
          ILogger.info(
              "Twitee", "Copy file from ${file.path} to ${newDir.path}");
  ...
  static Future<void> downloadAndUpdate(
    ...
    if (Utils.isNotEmpty(apkUrl)) {
      ...
      try {
        ...
        ILogger.error("Twitee", "Failed to download apk", e, t);
  ...
  static Future<ReleaseAsset> getAndroidAsset(
    ...
    ReleaseAsset generalAsset = assets.firstWhere((element) {
      return [
        'Twitee-$latestVersion.apk',
        'Twitee-$latestVersion-android-universal.apk'
  ...
  static WindowsVersion checkWindowsVersion() {
    ...
    if (result == WIN32_ERROR.ERROR_SUCCESS) {
      ...
      if (queryResult == WIN32_ERROR.ERROR_SUCCESS) {
        ...
        final installPath =
            "${installPathPtr.cast<Utf16>().toDartString()}\\Twitee.exe";
        ILogger.info("Twitee",
