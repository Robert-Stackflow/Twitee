...
class UrlPreviewHelper {
  ...
  static Future<void> showUrlPreviewOverlay(
    ...
    if (UriUtil.isTwitterUrl(url) || _currentUrl == url || _loading) return;
    _currentUrl = url;
    _loading = true;
    await remove();
    Metadata? data = await IToast.showLoadingSnackbar("正在获取网站预览", () async {
      try {
        ...
        ILogger.error(
            "UrlPreviewHelper", "Failed to extract metadata for $url", e, t);
      } finally {
        _loading = false;
        _currentUrl = "";
      }
...
//     RenderBox renderBox = rootContext.findRenderObject() as RenderBox;
